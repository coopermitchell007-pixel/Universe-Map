// ============================================================
// Tech Map — a separate 3D map of near-future engineering:
// SpaceX, Tesla, and the big ideas (fusion, space elevators,
// Dyson swarms…). Toggled with the ⚡ Tech button; the news and
// video drawers switch to tech content while it's open.
// ============================================================
...
const CLUSTERS = [
  {
    id: 'spacex', name: 'SPACEX', color: 0xffb070, pos: [-85, 8, -10],
    ...
    nodes: [
      {
        id: 'starship', name: 'Starship', sub: 'The largest flying object ever built',
      ...
      {
        id: 'falcon9', name: 'Falcon 9', sub: 'The rocket that changed everything',
      ...
      {
        id: 'raptor', name: 'Raptor Engine', sub: 'The most advanced rocket engine flying',
      ...
      {
        id: 'starlink', name: 'Starlink', sub: 'The largest satellite constellation in history',
      ...
      {
        id: 'dragon', name: 'Dragon', sub: 'America\'s crew spacecraft',
      ...
      {
        id: 'mars-plan', name: 'The Mars Plan', sub: 'A self-sustaining city by ~2050?',
  ...
  {
    id: 'tesla', name: 'TESLA', color: 0xff8080, pos: [80, 0, -25],
    ...
    nodes: [
      {
        id: 'fsd', name: 'Full Self-Driving (FSD)', sub: 'A robot you can sit inside',
      ...
      {
        id: 'optimus', name: 'Optimus', sub: 'A humanoid robot for boring, dangerous work',
      ...
      {
        id: 'energy', name: 'Megapack & Energy', sub: 'Grid-scale batteries — the quiet giant',
      ...
      {
        id: 'cybertruck', name: 'Cybertruck', sub: 'The polarising stainless-steel pickup',
  ...
  {
    id: 'future', name: 'FUTURE TECH', color: 0x7ae8ff, pos: [-5, -12, 85],
    ...
    nodes: [
      {
        id: 'fusion', name: 'Fusion Power', sub: 'Star power, bottled — closer than ever',
      ...
      {
        id: 'space-elevator', name: 'Space Elevator', sub: 'A cable to orbit — no rocket required',
      ...
      {
        id: 'dyson', name: 'Dyson Swarm', sub: 'Harvesting an entire star',
      ...
      {
        id: 'asteroid-mining', name: 'Asteroid Mining', sub: 'The first trillionaires will mine the sky',
      ...
      {
        id: 'oneill', name: "O'Neill Cylinders", sub: 'Cities in space — gravity included',
      ...
      {
        id: 'nuclear-prop', name: 'Nuclear Propulsion', sub: 'Mars in 45 days',
      ...
      {
        id: 'starshot', name: 'Breakthrough Starshot', sub: 'A probe to another star, this century',
      ...
      {
        id: 'terraforming', name: 'Terraforming Mars', sub: 'Planetary engineering — the 1,000-year project',
      ...
      {
        id: 'neuralink-node', name: 'Brain Interfaces', sub: 'Typing with thoughts — already happening',
      ...
      {
        id: 'ai-explore', name: 'AI in Space Exploration', sub: 'The first explorers won\'t be human',
...
export function initTech(ctx) {
  ...
  for (const cl of CLUSTERS) {
    ...
    hub.position.set(...cl.pos);
    const hubColor = new THREE.Color(cl.color);
    const rgba = `rgba(${hubColor.r * 255 | 0},${hubColor.g * 255 | 0},${hubColor.b * 255 | 0},1)`;

    const core = new THREE.Mesh(
      new THREE.IcosahedronGeometry(6, 1),
      new THREE.MeshBasicMaterial({ color: cl.color, wireframe: true, transparent: true, opacity: 0.85 }));
    const glow = new THREE.Sprite(new THREE.SpriteMaterial({
      map: TEX.glowTexture(rgba), transparent: true, opacity: 0.7, depthWrite: false }));
    glow.scale.setScalar(26);
    glow.userData.noPick = true;
    const lb = TEX.makeLabel(cl.name, { size: 9, color: '#' + hubColor.getHexString(), sub: cl.sub });
    lb.position.y = 13;
    hub.add(core, glow, lb);
    hub.userData.info = { id: 'tech-' + cl.id, name: cl.name, subtitle: cl.sub, stats: cl.stats, desc: cl.desc };
    hub.userData.focusable = true;
    hub.userData.focusDist = 60;
    hub.userData.pickPriority = 1;
    group.add(hub);
    pickables.push(hub);
    spinners.push(core);
    ...
    cl.nodes.forEach((nd, i) => {
      ...
      const a = (i / cl.nodes.length) * Math.PI * 2 + cl.pos[0] * 0.01;
      const r = 26 + (i % 3) * 9;
      const y = ((i * 0.618) % 1 - 0.5) * 26;
      node.position.set(Math.cos(a) * r, y, Math.sin(a) * r);

      const ball = new THREE.Mesh(
        new THREE.SphereGeometry(2.5, 18, 14),
        new THREE.MeshBasicMaterial({ color: cl.color, transparent: true, opacity: 0.9 }));
      const ng = new THREE.Sprite(new THREE.SpriteMaterial({
        map: TEX.glowTexture(rgba), transparent: true, opacity: 0.55, depthWrite: false }));
      ng.scale.setScalar(9);
      ng.userData.noPick = true;
      const nlb = TEX.makeLabel(nd.name, { size: 3.4, color: '#e8f2ff' });
      nlb.position.y = 5;
      node.add(ball, ng, nlb);
      node.userData.info = { id: 'tech-' + nd.id, name: nd.name, subtitle: nd.sub, stats: nd.stats, desc: nd.desc };
      node.userData.focusable = true;
      node.userData.focusDist = 22;
      ...
      const line = new THREE.Line(
        ...
        new THREE.LineBasicMaterial({ color: cl.color, transparent: true, opacity: 0.22 }));
  ...
  for (let i = 0; i < CLUSTERS.length; i++) {
    const a = new THREE.Vector3(...CLUSTERS[i].pos);
    const b = new THREE.Vector3(...CLUSTERS[(i + 1) % CLUSTERS.length].pos);
    const line = new THREE.Line(
  ...
  function enter() {
    ...
    setTimeout(() => {
      ...
      showToast('⚡ TECH MAP', 'SpaceX · Tesla · the engineering of the future — click any node');
  ...
  return {
    ...
    update(dt) {
      ...
      for (const s of spinners) { s.rotation.x += dt * 0.2; s.rotation.y += dt * 0.3; }
    },
  };
}
